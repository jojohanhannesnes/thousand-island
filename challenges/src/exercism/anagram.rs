...
pub fn anagrams_for<'a>(word: &str, possible_anagrams: &[&'a str]) -> HashSet<&'a str> {
    possible_anagrams
        .iter()
        .filter(|&&candidate| {
            // dbg!(candidate.len(), word.len());
            candidate.len() == word.len() && candidate.to_lowercase() != word.to_lowercase()
        })
        .filter(|&&candidate| {
            // dbg!(candidate);
            let is_contained = word
                .to_lowercase()
                .chars()
                .all(|x| candidate.to_lowercase().contains(x));

            let mut sort_word = word.to_lowercase().chars().collect::<Vec<_>>();
            let mut sort_candidate = candidate.to_lowercase().chars().collect::<Vec<_>>();
            sort_word.sort_unstable();
            sort_candidate.sort_unstable();
            is_contained && (sort_word == sort_candidate)
        })
        .cloned()
        .collect()
}
